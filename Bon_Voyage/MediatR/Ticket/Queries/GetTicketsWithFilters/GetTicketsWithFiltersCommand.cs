...
{
    public class GetTicketsWithFiltersCommand : IRequest<List<TicketsViewModel>>
    {
        public int StartIndex { get; set; }
        public TicketFilters Filters { get; set; }

        public class GetTicketsWithFiltersCommandHandler : BaseMediator, IRequestHandler<GetTicketsWithFiltersCommand,List<TicketsViewModel>>
        {
            public GetTicketsWithFiltersCommandHandler(EFDbContext context) : base(context)
            {
            }

            public async Task<List<TicketsViewModel>> Handle(GetTicketsWithFiltersCommand request, CancellationToken cancellationToken)
            {
                int take = 20;
                ...
                if (request.Filters != null)
                {
                    ...
                    return Filter(query, request.Filters)
                        .Skip(skip)
                        .Take(take)
